...
pub fn read_listview<O: Offset, R: Read + Seek>(
    ...
    Vec<u8>: TryInto<O::Bytes>,
{
    ...
    let field = ListViewArray::<O>::get_child_field(&data_type);
    ...
    ListViewArray::try_new(data_type, offsets.try_into()?, lengths.try_into()?, values, validity)
}

pub fn skip_listview<O: Offset>(
    field_nodes: &mut VecDeque<Node>,
    data_type: &ArrowDataType,
    buffers: &mut VecDeque<IpcBuffer>,
    variadic_buffer_counts: &mut VecDeque<usize>,
    ...
    let data_type = ListViewArray::<O>::get_child_type(data_type);
