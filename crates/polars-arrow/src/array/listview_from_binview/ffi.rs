...
use crate::array::listview::{ListViewElement, ViewType};
use crate::array::{FromFfi, ToFfi};
use crate::bitmap::align;
use crate::ffi;

unsafe impl<T: ViewType + ?Sized> ToFfi for ListViewArrayGeneric<T> {
...
impl<T: ViewType + ?Sized, A: ffi::ArrowArrayRef> FromFfi<A> for ListViewArrayGeneric<T> {
    unsafe fn try_from_ffi(array: A) -> PolarsResult<Self> {
        let data_type = array.data_type().clone();

        let validity = unsafe { array.validity() }?;
        let views = unsafe { array.buffer::<ListViewElement>(1) }?;
