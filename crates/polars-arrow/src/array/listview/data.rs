...
use crate::array::{from_data, to_data, Arrow2Arrow};
use super::ListViewArray;
use crate::bitmap::Bitmap;
use crate::offset::{Offset, OffsetsBuffer};

impl<O: Offset> Arrow2Arrow for ListViewArray<O> {
    fn to_data(&self) -> ArrayData {
        let data_type = self.data_type.clone().into();

        let builder = ArrayDataBuilder::new(data_type)
            .len(self.len())
            .buffers(vec![self.offsets.clone().into_inner().into(), self.lengths.clone().into_inner().into()])
    ...
    fn from_data(data: &ArrayData) -> Self {
        ...
        let mut lengths = unsafe { OffsetsBuffer::new_unchecked(data.buffers()[1].clone().into()) };

        offsets.slice(data.offset(), data.len() + 1);
        lengths.slice(data.offset(), data.len() + 1);
